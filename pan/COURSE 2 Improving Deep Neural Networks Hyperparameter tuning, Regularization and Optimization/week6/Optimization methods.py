...
def random_mini_batches(X, Y, mini_batch_size = 64, seed = 0):
    ...
    #将X中的元素根据随机列表的下标移动到新的位置，达到打乱X元素的目的
    shuffled_X = X[:, permutation]
    shuffled_Y = Y[:, permutation].reshape((1,m))#使用reshape保证shuffled_Y与Y的shape一致
...
def update_parameters_with_adam(parameters, grads, v, s, t, learning_rate = 0.01,
    ...
    for l in range(L):
        ...
        #s["db" + str(l+1)] = beta2*s["db"+str(l+1)]+(1-beta2)*grads["db"+str(l+1)] #❌ 严重错误。导致 b1为nan，进而导致Adam算法的cost为nan
